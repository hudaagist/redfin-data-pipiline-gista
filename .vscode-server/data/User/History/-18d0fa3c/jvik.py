...
def extract_data(**kwargs):
    ...
    now = datetime.now()
    date_now_string = now.stftrftime("%Y%m%d-%H%M%S")
...
def transfrom_data(task_instance):
    ...
    cols = [
        'period_begin', 'period_end', 'city', 'state', 'stae_code', 'property_type',
    ...
    s3_client.put_object(Bucket = 'redfin-data-bucket-gistack', Key = object_key, Body = csv_data)
